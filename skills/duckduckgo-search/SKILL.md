...
description: ⚠️ DEPRECATED — Use web-claude (통합 검색) instead. This skill has been merged into the unified web search strategy (web_search → duckduckgo → claude.ai).
allowed-tools: Bash(duckduckgo-search:*), Bash(python:*), Bash(pip:*), Bash(uv:*)
author: 무펭이 🐧
---

# ⚠️ DEPRECATED: DuckDuckGo Web Search Skill

**이 스킬은 더 이상 권장되지 않습니다.**

## 👉 대신 사용하세요: `web-claude` (통합 검색)

`/skills/web-claude/` 스킬이 3단계 fallback 전략을 제공합니다:
1. **web_search (Brave)** — 빠르고 안정적
2. **duckduckgo-search** — 프라이버시 중심 (이 스킬)
3. **claude.ai 브라우저** — 분석+검색
