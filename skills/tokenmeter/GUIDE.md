...
실행 완료 시 `events/cost-alert-YYYY-MM-DD.json` 자동 생성:
...
```json
{
  ...
  "data": {
    ...
    "month_cost": "$1,246.55",
    "savings": "$1,146.55",
    "max_plan_cost": "$100.00",
    "top_models": [
      {"model": "claude-opus-4", "cost": "$743.35", "percentage": "59.6%"},
      {"model": "claude-sonnet-4", "cost": "$501.75", "percentage": "40.3%"}
    ]
  },
...
이 이벤트는 daily-report가 자동으로 수집하여 "💰 비용/수익" 섹션에 포함합니다.

**생성 시점:**
- `tokenmeter dashboard` 실행 후
- `tokenmeter summary --period week` 또는 `--period month` 실행 후
- 스크립트로 자동화 가능 (cron 연동)
...
### 2026-02-14 (v4)
- ✅ 로우레벨 run.sh 구조로 전환
- ✅ SKILL.md → GUIDE.md 이동

### 2026-02-14 (v3)
- ✅ 이벤트 버스 연동: `cost-alert` 이벤트 자동 생성
- ✅ daily-report 자동 수집 지원
